...
import InputGeneral2 from "../Components/InputGeneral2";
...
const baseUrlCampo = "https://localhost:44366/Campo/RecCampo";
...
const Campo = () => {
 ...
 const expresionesRegulares = {
  ...
  const endPointValidarClave = 
    "https://localhost:44366/Usuario/ValidarUsuarioLogin/${IdUsuario.campo}/${Clave.campo}";
  ...
  function ValidarClave(){
    console.log("entro al metodo vaidar clarve");
    function showError() {
      Swal.fire({
        icon: "error",
        title: "Cuidado",
    ...
    const MetodoValidar = async () => {
      await axios.get(endPointValidarClave).then((response) => {
        const data = response.data;
        if (data === null) {
          console.log("entro al if donde la clave no conincide");
          cambiarClave({ campo: "IdUsuario.campo", valido: "false" });
          showError();
        } else {
          console.log("entro al if donde la clave si conincide");
  ...
  //   const url = `${baseUrlDel}/${options.idUsuario}`;
  ...
  const peticionDelete = async () => {
    // Creas el objeto con el IdUsuario
    console.log('nueva');
    // const options = {
    //   IdUsuario: IdUsuario.campo;
      ...
      <div className="relleno-general">
        ...
        <div className="container-fluid">
          <Formulario>
            <Columna>
            {data.map((item, index) => {
              console.log("item", item.estado);
              return (
                <InputGeneral2
                xestado={item.estado}
                xcambiarEstado={item.cambiarEstado}
                tipo={item.tipo}
                label={item.xLabel}
                placeholder={item.placeholder}
                name={item.xName}
                leyendaError={item.leyendaError}
                expresionRegular={item.expresionRegular}
              />
              );
            })}
